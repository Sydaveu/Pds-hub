    ...
    <div className="space-y-20">
        ...
        <div className="relative z-10 w-full max-w-4xl mx-auto text-center py-20">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <Link to="/categories" className="flex items-center gap-2 bg-white/5 backdrop-blur-sm hover:bg-white/10 text-white font-semibold py-4 px-8 rounded-2xl border border-white/10 hover:border-white/20 transition-all duration-300 hover:-translate-y-1">
                Explore Categories
              </Link>
            </div>
          </motion.div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map(({ value, label, icon: Icon, color }) => (
            <motion.div key={label} variants={item} className="glass-card rounded-2xl border border-white/5 p-6 text-center hover:border-purple-500/20 transition-all">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-400 max-w-xl mx-auto">Handpicked quality products from across all categories</p>
        </div>
        ...
        <div className="mt-10 text-center">
          <Link to="/marketplace" className="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-500 text-white font-semibold py-3 px-8 rounded-xl transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-purple-500/25">
          ...
          {[
            ...
            <motion.div key={step} variants={item} className="glass-card rounded-2xl border border-white/5 p-8 text-center hover:border-purple-500/20 transition-all">
              <div className="relative mb-5">
                <div className="w-16 h-16 bg-purple-600/10 rounded-2xl flex items-center justify-center mx-auto text-3xl">{icon}</div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-400">Three main categories with hundreds of products</p>
        </div>
        <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {categoryHighlights.map(cat => (
            <motion.div key={cat.id} variants={item}>
              <Link to={`/categories`} onClick={() => { sessionStorage.setItem('pds_active_main_cat', cat.id); }}
                className={`group relative overflow-hidden rounded-3xl h-48 flex flex-col items-center justify-center text-center bg-gradient-to-br ${cat.color} border border-white/10 hover:border-white/20 transition-all`}
              >
                {cat.image && (
                  <div className="absolute inset-0 bg-cover bg-center opacity-20 transition-all duration-500 group-hover:opacity-30 group-hover:scale-110"
        ...
        <div className="mt-10 text-center">
          <Link to="/categories" className="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-500 text-white font-semibold py-3 px-8 rounded-xl transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-purple-500/25">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <Link to="/marketplace" className="inline-flex items-center gap-2 bg-amber-400 hover:bg-amber-300 text-gray-900 font-bold py-4 px-10 rounded-2xl transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-amber-400/30">
